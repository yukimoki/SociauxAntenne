...
for idx, carre in df_carres[["IdSupProchePt1", "IdSupProchePt2", "IdSupProchePt3", "IdSupProchePt4"]].iterrows():
    ...
    densite_pop_carre = pop_total/40
    stat_for_pt = [pop_total, densite_pop_carre]
    for id_sup in carre:
        hab_by_holder[id_sup] = [hab_by_holder.setdefault(id_sup, [0, 0, 0])[i] + stat_for_pt[i] for i in range(len(stat_for_pt))]
    bar.next()
bar.finish()

df_stat = pd.DataFrame.from_dict(data=hab_by_holder, columns=['POP_TOTAL', 'DENSITE_POP_CARRE'], orient='index')
print(df_stat)
...
print(df_stat)
titre = sys.argv[1]
x_label = "Densité de la population des carrés (hab/km2)"
x = 'DENSITE_POP_CARRE'
y_label = "Population pauvre"
y = "POP_PAUVRE"
bins = 300
